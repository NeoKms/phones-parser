const megafonController = require('../controllers/mobile/megafon');
const logger = require('../helpers/logger');
const db = require('../db');
const {readerFromMegafon} = require("../controllers/rabbitmq/readers");
const {sleep} = require("../helpers/helper");

/**
 *  слушает очередь мегафон и парсит или меняет пароль
 *
 *  @param phone [string]
 ...
 */
readerFromMegafon(async function (msg, data) {
    let self = this;
    let msgtoerr = JSON.stringify(msg, null, 2)
    try {
        const systemIsOnline = await db.appConfig.working();
        if (!systemIsOnline) {
            logger.info('rabbit readerFromMegafon', "Система выключена");
            await sleep(1000*60);
            return self.nack(data);
        }
        logger.info(msg, 'rabbit readerFromMegafon');
        ...
        if (msg.type === "parse") {
            const {profile, balance, tariff, GB} = await megafonController.parseAllData(phoneObj);
            ...
            await megafonController.changePassword(phoneObj, passObj.value);
        ...
        logger.info('finished readerFromMegafon')
    }
});
